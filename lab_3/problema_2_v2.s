...
// Mueve el Vector de Interrupciones al principio de la 2da RAM
    ...
    MOV R0,#(SCU_MODE_PULLUP | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_EN | SCU_MODE_FUNC4)
...
refrescar:
    @ TST     R3,#0x00                // Me fijo si hay algún bit en 1
    @ IT      NE
    @ MOVNE   R3,#0x00                // Define el estado actual de los segmentos como todos apagados
    LDR     R3,[R4,#SEG_OFFSET]     // Carga el estado actual de las teclas
    ...
    EOREQ   R3,#SEG_B_MASK          // Invierte el bit del segmento B

    // Enciende/apaga el segmento C si se aprieta el boton 2
    TEQ     R0,#BOTON_2_MASK
    IT      EQ
    EOREQ   R3,#SEG_C_MASK

    // Enciende/apaga el segmento E si se aprieta el boton 3
    TEQ     R0,#BOTON_3_MASK
    IT      EQ
    EOREQ   R3,#SEG_E_MASK

    // Enciende/apaga el segmento F si se aprieta el boton 4
    TEQ     R0,#BOTON_4_MASK
    IT      EQ
    EOREQ   R3,#SEG_F_MASK
