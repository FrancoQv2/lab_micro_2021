...
timer_init:
    ...
    // Prescaler de 9.500.000 para una frecuencia de 10 Hz, 95.000 para 1 seg
    LDR R0,=950000
    STR R0,[R1,#PR]
    //----------------------
...
.endfunc
    ...
    .func refrescar_pantalla
refrescar_pantalla:
    PUSH {LR}
    PUSH {R5}

    BL   apagar_digitos
    BL   refrescar_digitos

    MOV  R5,R0                      // Salvo el numero de refresco
    LDR  R1,=hora
    LDRB R0,[R1,R0]                 // Guardo en R0 el numero BCD que quiero convertir
    BL   convertir_7seg    // Devuelve en R0 el display 7 segmento

    BL   encender_segmentos
...
//SUBRUTINAS DE SEGMENTOS
    ...
    .func encender_segmentos
encender_segmentos:
    //Limpio lo que haya en el segmento (APAGO TODO)
    LDR R1,=GPIO_CLR0
    LDR R2,=SEG_N_MASK
    STR R2,[R1,#SEG_N_OFFSET]

    //Cargo el segmento
    LDR R1,=GPIO_PIN0
    LDR R2,[R1,#SEG_N_OFFSET] //TRAIGO LO QUE YA ESTA EN EL GPIO
    ORR R0,R2               //HAGO UN OR entre lo que quiero mostrar y lo que habia en el GPIO
    STR R0,[R1,#SEG_N_OFFSET] //Cargo el segmento que quiero que se vea
...
apagar_digitos:
    ...
    .func encender_digito
encender_digito:
    PUSH {LR}
    BL   seleccionar_digito
    LDR  R1,=GPIO_PIN0
    LDR  R2,[R1,#DIG_OFFSET]    // TRAIGO LO QUE YA ESTA EN EL GPIO
    // HAGO UN OR entre la MASK del digito que quiero y lo que habia en el prender digito
    ORR  R0,R2                  
    STR  R0,[R1,#DIG_OFFSET]
...
segmentos_init:
    ...
    LDR R0,=GPIO_CLR0
    LDR R1,=SEG_N_MASK
    STR R1,[R0,#SEG_N_OFFSET]
    //-----

    //Configuro como Salidas
    LDR R0,=GPIO_DIR0
    LDR R1,[R0,#SEG_N_OFFSET]
    ORR R1,#SEG_N_MASK            //salida si DIR =1 y entrada si Dir=0 por eso or
    STR R1,[R0,#SEG_N_OFFSET]
