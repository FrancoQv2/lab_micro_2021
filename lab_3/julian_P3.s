...
reset:
    ...
    // Esto no hace falta aquí porque no lo hago por interrupciones
    @ // Configuro los pines para operacion por flancos
    @ LDR R4,=PINT_BASE
    @ MOV R0,#0x00
    @ STR R0,[R4,#ISEL]           // 0 en ISEL p/ operaciones por flanco; 1 por nivel
    @ MOV R0,#0xFF
    @ STR R0,[R4,#CIENR]          // 1 en CIENR para un 0 en IENR (0 inhabilita flanco ascendente)
    @ STR R0,[R4,#CIENF]          // 1 en CIENF para un 0 en IENF (0 inhabilita flanco descendente)
    @ MOV R0,#0x03
    @ STR R0,[R4,#SIENR]          // 0000.0011 en SIENR para 1 en IENR (habilitar flanco ascendente)
    @ MOV R0,#0x05
    @ STR R0,[R4,#SIENF]          // 0000.0101 en SIENF para 1 en IENF (habilitar flanco descendente)
